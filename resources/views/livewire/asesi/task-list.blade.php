<div class="flex-1 ml-0 md:ml-64 pt-10 px-6 md:px-12 pb-12 min-h-screen bg-gray-100">
    <!-- Header Section -->
    @if ($batch)
        <div class="mb-10 max-w-8xl mx-auto">
            ...
            <div class="flex border-b border-gray-300 mb-8 overflow-x-auto hide-scrollbar">
                <button wire:click="changeTab('belum_dikerjakan')"
                    class="px-6 py-4 text-sm uppercase tracking-wider font-semibold flex items-center gap-2 whitespace-nowrap {{ $activeTab === 'belum_dikerjakan' ? 'text-cyan-700 border-b-2 border-cyan-700' : 'text-brandBlue-dark' }}">
                ...
                <button wire:click="changeTab('sudah_dikirim')"
                    class="px-6 py-4 text-sm uppercase tracking-wider font-semibold flex items-center gap-2 whitespace-nowrap {{ $activeTab === 'sudah_dikirim' ? 'text-cyan-700 border-b-2 border-cyan-700' : 'text-brandBlue-dark' }}">
                    <span>Sudah Dikirim</span>
                    @if (count($sentTasks) > 0)
                        <span
                            class="px-2 py-0.5 text-xs rounded-full {{ $activeTab === 'sudah_dikirim' ? 'bg-gray-500 text-white font-bold' : 'bg-gray-200 text-gray-700 font-medium' }}">
                            {{ count($sentTasks) }}
                        </span>
                    @endif
                </button>
                ...
                <button wire:click="changeTab('sudah_dikerjakan')"
                    class="px-6 py-4 text-sm uppercase tracking-wider font-semibold flex items-center gap-2 whitespace-nowrap {{ $activeTab === 'sudah_dikerjakan' ? 'text-cyan-700 border-b-2 border-cyan-700' : 'text-brandBlue-dark' }}">
                    <span>Sudah Selesai</span>
                    @if (count($doneTasks) > 0)
                        <span
                            class="px-2 py-0.5 text-xs rounded-full {{ $activeTab === 'sudah_dikerjakan' ? 'bg-emerald-500 text-white font-bold' : 'bg-emerald-100 text-emerald-700 font-medium' }}">
                            {{ count($doneTasks) }}
                        </span>
                    @endif
                </button>
            ...
            @if (session()->has('success'))
                <div
                    class="mb-6 bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-r-xl shadow-sm flex items-center gap-3 animate__animated animate__fadeIn">
            ...
            @if (session()->has('error'))
                <div
                    class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl shadow-sm flex items-center gap-3 animate__animated animate__fadeIn">
            ...
            <div class="grid grid-cols-1 gap-8">
                ...
                <div class="lg:col-span-8 flex flex-col gap-6">
                    ...
                    @if ($activeTab === 'belum_dikerjakan')
                                ...
                                <div class="max-w-none">
                                    ...
                                    @if ($data->image_path)
                                        <div class="mt-4 mb-6">
                                            <a href="{{ asset('storage/' . $data->image_path) }}" target="_blank"
                                                class="block group relative overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm max-w-md transition-all hover:shadow-md">
                                                <img src="{{ asset('storage/' . $data->image_path) }}"
                                                    alt="Gambar Penugasan"
                                                    class="w-full h-auto max-h-72 object-contain mx-auto transition-transform duration-300">
                                                <div
                                                    class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                                    <span
                                                        class="text-white text-sm font-semibold flex items-center gap-1.5 bg-black/60 px-4 py-2 rounded-full backdrop-blur-sm">
                                                        <span
                                    ...
                                    <div class="pt-8" x-data="{
                                        ...
                                        <div @click="triggerSelect" @dragover.prevent="isDragging = true"
                                            ...
                                            :class="isDragging ? 'border-brandBlue bg-blue-100/50' :
                                                'border-outline-variant bg-blue-50'"
                                            ...
                                            <input accept=".pdf,.doc,.docx" class="hidden" x-ref="fileInput"
                                                @change="handleFileSelect" type="file">
                                            ...
                                            <template x-if="!fileName">
                                                <div class="text-center">
                                                    <p class="text-gray-800 font-medium mb-1">Drag &amp; drop file Anda
                                                        di sini</p>
                                                    <p class="text-slate-500 text-xs mb-3">Maksimal ukuran file: 10MB
                                                        (PDF, Word)</p>
                                                    <p class="text-gray-800 text-sm mb-2">atau</p>
                                                    <button type="button"
                                                        class="px-6 py-2 rounded-lg border border-outline-variant bg-white text-gray-800 font-medium hover:bg-surface-container-high transition-colors text-sm">
                                            ...
                                            <template x-if="fileName">
                                                <div class="text-center w-full max-w-xs">
                                                    <div class="flex items-center justify-center gap-2 mb-2">
                                                        <span
                                                            class="material-symbols-outlined text-brandBlue">description</span>
                                                        <span class="text-gray-800 font-semibold truncate text-sm"
                                                    ...
                                                    <template x-if="isUploading">
                                                        <div
                                                            class="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700 mt-2">
                                                            <div class="bg-brandBlue h-1.5 rounded-full"
                                                    ...
                                                    <template x-if="!isUploading">
                                                        <span
                                                            class="text-xs text-brandGreen font-medium flex items-center justify-center gap-1">
                                                            <span
                                        ...
                                        <div class="mt-4 flex justify-end" x-show="fileName" x-transition>
                                            <button wire:click="submitTask({{ $data->id }})"
                        ...
                        @empty
                            <x-asesi.empty-state-card title="Tugas Belum Tersedia"
                    ...
                    @elseif ($activeTab === 'sudah_dikirim')
                        @forelse ($sentTasks as $submission)
                            <div class="bg-white rounded-xl p-8 border-ghost mb-10">
                                ...
                                {{-- <div class="bg-emerald-50 border-l-4 border-emerald-500 p-4 mb-6 rounded-r-xl">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-emerald-700">Anda masih dapat memperbarui jawaban
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                                    ...
                                    <div
                                        ...
                                        <p class="text-sm font-label text-white font-medium">
                                            Batas Pengumpulan: <span
                                                class="font-bold text-accent">{{ $submission->task->ends_at->timezone('Asia/Makassar')->locale('id')->isoFormat('dddd, D MMMM YYYY, HH:mm') }}
                                                WITA</span>
                                        </p>
                                    </div>
                                </div>
                                ...
                                @if ($submission->task->image_path)
                                    <div class="mt-4 mb-6">
                                        <a href="{{ asset('storage/' . $submission->task->image_path) }}"
                                            target="_blank"
                                            class="block group relative overflow-hidden rounded-xl border border-gray-200 bg-gray-50 shadow-sm max-w-md transition-all hover:shadow-md">
                                            <img src="{{ asset('storage/' . $submission->task->image_path) }}"
                                                alt="Gambar Penugasan"
                                                class="w-full h-auto max-h-72 object-contain mx-auto transition-transform duration-300 group-hover:scale-[1.02]">
                                            <div
                                                class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                                <span
                                                    class="text-white text-sm font-semibold flex items-center gap-1.5 bg-black/60 px-4 py-2 rounded-full backdrop-blur-sm">
                                ...
                                <div>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">Dikirim pada:
                                                    {{ $submission->submitted_at->timezone('Asia/Makassar')->locale('id')->isoFormat('D MMMM YYYY, HH:mm') }}
                                                    WITA</p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-4 text-xs text-gray-600 flex items-center gap-1.5">
                                        ...
                                        <span>Jumlah pengumpulan: <strong
                                                class="text-gray-800">{{ $submission->submission_count }} dari
                                                {{ $submission->task->max_submissions }} kali</strong> batas
                                            maksimal.</span>
                                    </div>
                                </div>
                                ...
                                <div class="pt-6 border-t border-gray-100" x-data="{
                                    ...
                                    {{-- <div @click="triggerSelect" @dragover.prevent="isDragging = true"
                                        ...
                                        <div
                                            class="rounded-full bg-white shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform duration-300 w-10 h-10 mb-2 border border-gray-100">
                                            <span
                                                class="material-symbols-outlined text-brandBlue text-xl">cloud_upload</span>
                                        </div>

                                        <template x-if="!fileName">
                                            <div class="text-center">
                                                <p class="text-gray-800 text-sm font-medium mb-1">Drag &amp; drop
                                                    berkas baru di sini untuk mengganti</p>
                                                <p class="text-gray-400 text-xs mb-2">Maksimal ukuran file: 10MB (PDF,
                                                    Word)</p>
                                                <button type="button"
                                                    class="px-4 py-1.5 rounded-lg border border-gray-300 bg-white text-gray-700 font-medium hover:bg-gray-50 transition-colors text-xs shadow-sm">Pilih
                                                    Berkas</button>
                                            </div>
                                        </template>
                                    ...
                                    <div class="mt-4 flex justify-center" x-show="fileName" x-transition>
                                        <button wire:click="submitTask({{ $submission->task_id }})"
                                            :disabled="isUploading"
                                            :class="isUploading ? 'opacity-50 cursor-not-allowed' : 'hover:brightness-110'"
                        ...
                        @empty
                            <x-asesi.empty-state-card title="Tidak Ada Tugas Berjalan"
                    ...
                    @elseif ($activeTab === 'sudah_dikerjakan')
                        ...
                        @forelse ($doneTasks as $submission)
                            <div class="bg-white rounded-xl p-8 border-ghost mb-10">
                                ...
                                <div
                                    class="flex items-center justify-between bg-white p-4 rounded-lg border border-gray-150 shadow-sm">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 bg-red-50 text-red-500 rounded flex items-center justify-center border border-red-100 font-bold text-xs uppercase">
                                            {{ pathinfo($submission->file_path, PATHINFO_EXTENSION) }}
                                ...
                                <div class="mt-4 text-xs text-gray-500 flex items-center gap-1.5">
                                    ...
                                    <span>Status pengiriman: <strong class="text-emerald-600">Final (Selesai)</strong>
                                        dengan <strong class="text-gray-800">{{ $submission->submission_count }} dari
                                            {{ $submission->task->max_submissions }} kali</strong> pengumpulan.</span>
                                </div>
                            </div>
                        @empty
                            <x-asesi.empty-state-card title="Belum Ada Tugas Final"
                                message="Anda belum memiliki tugas yang sudah dikirimi secara final." icon="lock"
                                type="info" />
                        @endforelse
                    @endif
