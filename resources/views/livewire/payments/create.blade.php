            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-3xl p-8 shadow-lg hover-lift">
                    ...
                    <div>
                        ...
                        @if ($mode === 'bundle')
                            <div class="mb-8">
                                <div class="relative bg-gradient-to-br from-teal-500 via-teal-600 to-cyan-600 rounded-2xl p-6 text-white overflow-hidden">
                                    {{-- Decorative Elements --}}
                                    ...
                                    <div class="absolute top-4 right-4">
                                        <span class="bg-amber-400 text-amber-900 text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                                            HEMAT 23%
                                        </span>
                                    </div>
                                    ...
                                    <div class="relative z-10">
                                        <div class="flex items-start gap-4 mb-4">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-white/80 text-sm">Akses penuh ke semua kategori ujian Level A</p>
                                            </div>
                                        </div>
                                        
                                        {{-- Included Items --}}
                                        <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 mb-4">
                                            <p class="text-xs text-white/70 uppercase tracking-wide mb-3 font-semibold">Termasuk dalam paket:</p>
                                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                                @foreach($categories as $cat)
                                                <div class="flex items-center gap-2">
                                                    <svg class="w-5 h-5 text-teal-200" fill="currentColor" viewBox="0 0 20 20">
                                        ...
                                        <div class="flex items-end justify-between">
                                            <div>
                                                <div class="flex items-center gap-2 mb-1">
                                                    <span class="text-white/60 line-through text-sm">Rp {{ number_format(count($categories) * $categoryPrice, 0, ',', '.') }}</span>
                                                    <span class="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">-Rp {{ number_format($this->savings, 0, ',', '.') }}</span>
                                                </div>
                                                <div class="flex items-baseline gap-1">
                                                    <span class="text-3xl font-extrabold">Rp {{ number_format($bundlePrice, 0, ',', '.') }}</span>
                        ...
                        @if ($mode === 'custom')
                            <div class="mb-8 space-y-4">
                                ...
                                @if(count($selectedCategories) === 2)
                                    <div class="flex items-center justify-between bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="text-sm font-semibold text-amber-800">
                                                    <span class="font-bold">Hemat lebih banyak!</span> Tambah 2 kategori lagi untuk dapat Paket Level A seharga Rp 150.000
                                                </p>
                                                <p class="text-xs text-amber-600">(hemat Rp {{ number_format($this->savings, 0, ',', '.') }})!</p>
                                            </div>
                                        </div>
                                        <button wire:click="switchMode('bundle')" class="flex-shrink-0 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
                                            Upgrade Paket
                                        </button>
                                    </div>
                                @endif
                                    ...
                                    <div wire:click="toggleCategory('{{ $cat['id'] }}')"
                                        ...
                                        <div class="w-12 h-12 mb-4 rounded-xl flex items-center justify-center shadow-lg"
                                            ...
                                            @elseif($cat['id'] === 'numerasi')
                                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a5 5 0 00-10 0v2a2 2 0 00-2 2v5a2 2 0 002 2h10a2 2 0 002-2v-5a2 2 0 00-2-2z"/></svg>
                                            @elseif($cat['id'] === 'pck')
                                        ...
                                        <p class="text-sm text-gray-500 mb-4 leading-relaxed">{{ $cat['desc'] }}</p>
                                        <!-- Tags -->
                                        <div class="flex flex-wrap gap-2 mb-4">
                                            @foreach($cat['tags'] as $tag)
                                                <span class="text-xs bg-{{ $cat['color'] }}-100 text-{{ $cat['color'] }}-700 px-2.5 py-1 rounded-full font-medium">{{ $tag }}</span>
                                            @endforeach
                                        </div>
                                        <!-- Price -->
                                        <div class="flex items-baseline gap-1">
                                            <span class="text-2xl font-extrabold text-{{ $cat['color'] }}-600">Rp {{ number_format($categoryPrice, 0, ',', '.') }}</span>
                                ...
                                <div class="flex items-center justify-between pt-2">
                                    ...
                                    @if(count($selectedCategories) > 0)
                                        <button wire:click="selectAllCategories" class="flex items-center gap-2 text-sm text-teal-600 hover:text-teal-700 font-medium transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            Pilih semua kategori
                                        </button>
                                        <button wire:click="resetSelection" class="text-sm text-gray-500 hover:text-gray-700 font-medium transition-colors">
                                            Reset pilihan
                                        </button>
                                    @endif
                                </div>
                    ...
                    <div class="border-t border-gray-100 pt-8">
                        <button wire:click="toggleExpand" class="flex items-center justify-between w-full text-left group">
                            <span class="text-lg font-semibold text-gray-800 group-hover:text-teal-600 smooth-transition">
                                Detail Program & Modul
                            </span>
                            <svg class="w-5 h-5 text-teal-600 smooth-transition group-hover:text-teal-700 {{ $expandDetail ? 'arrow-rotate' : '' }}" fill="none"
                        ...
                        <div class="overflow-hidden smooth-transition @if($expandDetail) expanding @endif" style="max-height: {{ $expandDetail ? '1000px' : '0' }};">
                            @if($expandDetail)
                            ...
                            @if($mode === 'bundle')
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex flex-wrap gap-1.5">
                                        @foreach($categories as $cat)
                                            <span class="text-xs bg-teal-100 text-teal-700 px-2 py-0.5 rounded-full">{{ $cat['name'] }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            @else
                                ...
                                <div class="space-y-2">
                                    @if(count($selectedCategories) === 0)
                                        <p class="text-gray-400 italic text-sm">Pilih kategori untuk melihat ringkasan</p>
                                    @else
                                        @foreach($selectedCategories as $catId)
                                            <div class="flex justify-between items-center text-sm">
                                                ...
                                                <span class="font-medium text-gray-800">Rp {{ number_format($categoryPrice, 0, ',', '.') }}</span>
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
                        ...
                        <div class="space-y-3 mb-6 border-t border-gray-100 pt-6">
                            @if($mode === 'bundle')
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Harga Normal ({{ count($categories) }} kategori)</span>
                                    <span class="text-gray-400 line-through">Rp {{ number_format(count($categories) * $categoryPrice, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Diskon Paket</span>
                                    <span class="text-green-600 font-medium">- Rp {{ number_format($this->savings, 0, ',', '.') }}</span>
                                </div>
                            @else
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">Subtotal ({{ count($selectedCategories) }} kategori)</span>
                                    <span class="font-medium">Rp {{ number_format($this->totalPrice, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            <hr class="border-gray-200">
                            <div class="flex justify-between text-lg font-bold">
                                <span>Total</span>
                                <span class="gradient-text">Rp {{ number_format($this->totalPrice, 0, ',', '.') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-gradient-to-br from-teal-600 to-cyan-700 rounded-2xl p-5 text-white shadow-lg">
                            <div class="flex items-start gap-3 mb-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-white/80 mt-1">Paket Level A ({{ count($categories) }} kategori) = <span class="font-bold">Rp {{ number_format($bundlePrice, 0, ',', '.') }}</span></p>
                                    <p class="text-xs text-white/80">vs beli satuan {{ count($categories) }} kategori = <span class="line-through">Rp {{ number_format(count($categories) * $categoryPrice, 0, ',', '.') }}</span></p>
                                </div>
                            </div>
                            <button wire:click="switchMode('bundle')" class="w-full bg-white text-teal-700 font-semibold py-2.5 px-4 rounded-lg text-sm hover:bg-gray-50 transition-colors">
                                Lihat Paket Lengkap
                            </button>
                        </div>
                    @endif
