...
<head>
    ...
    <style>
        ...
        input:focus,
        select:focus {
            border-color: #0C548C !important;
            box-shadow: 0 0 0 3px rgba(12, 84, 140, 0.1);
        }
    </style>
</head>
    ...
    <div class="max-w-3xl mx-auto py-10 px-4 sm:px-6">
        ...
        <div class="flex items-center justify-center gap-0 mb-8">
            <div class="flex items-center gap-2">
                <div
                    class="w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-white text-sm font-bold hidden sm:flex">
                    ✓</div>
            ...
            <div class="flex items-center gap-2">
                <div
                    class="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center text-white text-sm font-bold hidden sm:flex">
                    2</div>
            ...
            <div class="flex items-center gap-2">
                <div
                    class="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center text-gray-500 text-sm font-bold hidden sm:flex">
                    3</div>
        ...
        <form action="{{ route('registeraddtionalpost') }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
                ...
                <div class="flex flex-col items-center">
                    <div class="relative group mb-3">
                        ...
                        <label for="profile_image"
                            class="absolute inset-0 flex items-center justify-center rounded-full bg-black/30 opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 section-card">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Lengkap / Gelar <span
                    ...
                    {{-- NIK --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">NIK <span
                                class="text-gray-400 font-normal">(opsional)</span></label>
                        <input type="text" name="nik" value="{{ old('nik') }}" placeholder="16 digit angka"
                            maxlength="16"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm transition-all duration-150 placeholder:text-inputHint"
                    ...
                    <div>
                        ...
                        <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir') }}"
                            ...
                            class="w-full border text-gray-700 border-gray-300 rounded-lg px-3 py-2.5 text-sm transition-all duration-150 placeholder:text-inputHint"
                            required>
                        @error('tempat_lahir')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    @if (!$hasNoWa)
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Nomor WhatsApp <span
                                    class="text-red-500">*</span></label>
                            <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                                <span
                                    class="bg-gray-100 px-3 py-2.5 text-sm text-gray-500 border-r border-gray-300">+62</span>
                                <input type="text" name="no_wa" value="{{ old('no_wa') }}"
                                    placeholder="81234567890" class="flex-1 px-3 py-2.5 text-sm border-0 focus:ring-0"
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 section-card">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {{-- Profesi --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Profesi <span
                                class="text-red-500">*</span></label>
                        <select name="profesi" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white transition-all duration-150 text-gray-700">
                            <option value="" disabled {{ old('profesi') ? '' : 'selected' }}>-- Pilih Profesi --
                            </option>
                            @foreach (['Guru', 'Dosen', 'Trainer / Instruktur', 'Kepala Sekolah', 'Pengawas / Supervisor', 'Tenaga Kependidikan', 'Mahasiswa', 'Lainnya'] as $p)
                                <option value="{{ $p }}" {{ old('profesi') == $p ? 'selected' : '' }}>
                                    {{ $p }}</option>
                            @endforeach
                        </select>
                        @error('profesi')
                    ...
                    {{-- Lama Masa Kerja --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Lama Masa Kerja <span
                                class="text-gray-400 font-normal">(opsional)</span></label>
                        <select name="lama_masa_kerja"
                            ...
                            @foreach (['< 1 tahun', '1-3 tahun', '4-6 tahun', '7-10 tahun', '> 10 tahun'] as $lm)
                                <option value="{{ $lm }}"
                                    {{ old('lama_masa_kerja') == $lm ? 'selected' : '' }}>{{ $lm }}</option>
                            @endforeach
                        </select>
                    ...
                    {{-- Instansi --}}
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Instansi / Sekolah <span
                                class="text-red-500">*</span></label>
                        <input type="text" id="instansi" name="instansi" value="{{ old('instansi') }}"
                            placeholder="Tuliskan nama instansi atau sekolah Anda" maxlength="50" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm transition-all duration-150 placeholder:text-inputHint">
                        @error('instansi')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 section-card">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <select name="latar_belakang_pendidikan"
                            ...
                            @foreach (['SMA/SMK', 'D1', 'D2', 'D3', 'D4', 'S1', 'S2', 'S3', 'Lainnya'] as $jenjang)
                                <option value="{{ $jenjang }}"
                                    {{ old('latar_belakang_pendidikan') == $jenjang ? 'selected' : '' }}>
                                    {{ $jenjang }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="tahun_studi" value="{{ old('tahun_studi') }}"
                    ...
                    {{-- Nama Universitas --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Universitas /
                            Sekolah</label>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 section-card">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {{-- Provinsi --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Provinsi <span
                                class="text-red-500">*</span></label>
                    ...
                    {{-- Kabupaten/Kota --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Kabupaten / Kota <span
                                class="text-red-500">*</span></label>
                    ...
                    {{-- Kelurahan --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Kelurahan / Desa <span
                                class="text-red-500">*</span></label>
            ...
            <div class="text-center text-sm text-gray-500 mt-4 pb-6">
                ...
                <button onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                    class="text-primary hover:underline font-semibold focus:outline-none">
                ...
                if (name) {
                    $.get('/regencies/' + encodeURIComponent(name), function(data) {
                        ...
                        $.each(data, function(i, v) {
                            opts += '<option value="' + v.name + '">' + v.name +
                            '</option>';
                        });
