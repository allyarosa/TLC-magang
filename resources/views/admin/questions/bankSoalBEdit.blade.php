...
@section('content')
    <div class="max-w-5xl mx-auto px-4 pt-6 mt-4">
        ...
        <div
            ...
            <h1 class="text-xl font-bold text-white sm:text-2xl">Edit Soal Level B</h1>
        </div>
        ...
        <div class="bg-white border border-gray-100 rounded-xl shadow-md overflow-hidden">
            ...
            <!-- Form body -->
            <div class="p-2">
                <form action="{{ route('admin.question.b.update', $question->id) }}" enctype="multipart/form-data" method="post">
                    @csrf
                    @method('put')
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <select id="category_b_id" name="category_b_id" onchange="showCustomInput()"
                                            ...
                                            @foreach ($categoriesB as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ old('category_b_id', $question->category_b_id) == $category->id ? 'selected' : '' }}>
                                                    {{ $category->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    {{-- <x-input-error :messages="$errors->get('category_b_id')" class="mt-1 text-xs" /> --}}
                                </div>

                                <!-- text soal -->
                                <div>
                                    <div class="mb-4">
                                        <label for="question_text" class="block mb-2 text-sm font-medium text-gray-700">Text
                                            Soal</label>
                                ...
                                <div>
                                    ...
                                    <select id="correct_answer" name="correct_answer"
                                        ...
                                        <option value="option_a" {{ old('correct_answer') == 'option_a' ? 'selected' : '' }}>Option A
                                        </option>
                                        <option value="option_b" {{ old('correct_answer') == 'option_b' ? 'selected' : '' }}>Option B
                                        </option>
                                        <option value="option_c" {{ old('correct_answer') == 'option_c' ? 'selected' : '' }}>Option C
                                        </option>
                                        <option value="option_d" {{ old('correct_answer') == 'option_d' ? 'selected' : '' }}>Option D
                                        </option>
                                    </select>
