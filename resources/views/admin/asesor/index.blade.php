...
@section('content')
                    ...
                    @forelse ($asesors as $asesor)
                        <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="h-10 w-10">
                                        <img class="h-10 w-10 rounded-full object-cover border-2 border-indigo-100"
                                            src="{{ $asesor->profile_image ? asset('storage/' . $asesor->profile_image) : asset('assets/img/blank_profile.png') }}"
                                            alt="Profile image">
                                    </div>
                            ...
                            <td class="px-4 py-3 text-sm text-blue-600 underline">
                                @if ($asesor->berkas_cv)
                                    <a href="{{ asset('storage/' . $asesor->berkas_cv) }}" target="_blank">Lihat CV</a>
                                @else
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500">
                                {{ $asesor->user->last_seen ?? '-' }}
                            </td>

                            <td class="px-4 py-3 text-sm text-gray-500">
                                <span
                                    class="px-2 py-1 rounded-full text-xs {{ $asesor->user->status == 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                    {{ ucfirst($asesor->user->status) }}
                                </span>
                            </td>

                            <td class="px-4 py-3 text-sm">
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.asesor.show', $asesor->id) }}"
                                        class="p-2 text-amber-600 bg-amber-50 rounded-md hover:bg-amber-100"
                                        title="Lihat Detail">
                                        👁️
                                    </a>
                                    ...
                                    <form action="{{ route('admin.asesor.destroy', $asesor->id) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="p-2 text-red-600 bg-red-50 rounded-md hover:bg-red-100" title="Hapus">
                                            🗑️
                                        </button>
                                    </form>
                                </div>
        ...
        <div class="relative w-full max-w-md max-h-full p-4">
            <div class="relative bg-white rounded-lg shadow-lg">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-medium text-gray-800">Apakah anda yakin ingin delete all asesor?</h3>
    ...
    <div data-popover id="popover-export" role="tooltip"
        ...
        <div class="px-3 py-2 bg-blue-50 border-b border-gray-200 rounded-t-lg">
            <h3 class="font-semibold text-blue-600">Export asesor</h3>
        </div>
        <div class="px-3 py-2">
            <p>Tindakan ini akan membuat file excel dari data asesor.</p>
        </div>
        <div data-popper-arrow></div>
    </div>
