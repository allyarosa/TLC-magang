                                    ...
                                    <div>
                                        <span
                                            ...
                                            <svg class="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                                    d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-8.293l-3-3a1 1 0 00-1.414 1.414L10.586 9.5 8.293 7.207a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0z"
                                                    clip-rule="evenodd" />
                                            </svg>
                                        </span>
                                    </div>
                                    <div class="min-w-0 flex-1 pt-1.5">
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">
                                                {{ $payment->created_at->format('d M Y, H:i:s') }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <li>
                                <div class="relative pb-8">
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5">
                                            <div>
                                                <p class="text-sm text-gray-900 font-medium">Payment
                                                    {{ ucfirst($payment->status) }}</p>
                                                <p class="text-sm text-gray-500">
                                                    {{ $payment->payment_time->format('d M Y, H:i:s') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="waiting_confirmation"
                                {{ $payment->status === 'waiting_confirmation' ? 'selected' : '' }}>Menunggu Konfirmasi
                            </option>
                            <option value="success" {{ $payment->status === 'success' ? 'selected' : '' }}>Success
                            </option>
                            <option value="failed" {{ $payment->status === 'failed' ? 'selected' : '' }}>Failed</option>
                            <option value="expired" {{ $payment->status === 'expired' ? 'selected' : '' }}>Expired
                            </option>
                        </select>
                    </div>
                    <button type="submit"
                        ...
                        @if ($payment->payment_type)
                            <div>
                                ...
                                <p class="text-sm text-gray-900 capitalize">
                                    {{ str_replace('_', ' ', $payment->payment_type) }}</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                @if ($payment->transfer_proof)
                    ...
                    <div class="relative group">
                        <img src="{{ asset('storage/' . $payment->transfer_proof) }}" alt="Bukti Transfer"
                    ...
                    <div class="mt-3 flex items-center justify-between">
                        ...
                        <a href="{{ asset('storage/' . $payment->transfer_proof) }}"
                            download="{{ Str::slug($payment->user->name ?? 'asesi') }}-bukti-pembayaran-{{ $payment->created_at->format('d-m-Y') }}.{{ pathinfo($payment->transfer_proof, PATHINFO_EXTENSION) }}"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-indigo-700 rounded-md hover:bg-indigo-100 transition-colors">
