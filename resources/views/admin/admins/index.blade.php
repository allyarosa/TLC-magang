...
@section('content')
        ...
        <nav class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-4 text-base">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="text-gray-600 text-sm">
                    Total : {{ $userCount['user'] }} Admin
                </div>

                <!-- Actions -->
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.admins.create') }}" data-popover-target="popover-addUser"
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-primary-500">
                    <tr>
                        ...
                        <th class="w-20 px-4 py-3 text-xs font-medium text-center text-white uppercase tracking-wider">
                            Foto
                        </th>
                        <th class="w-48 px-4 py-3 text-xs font-medium text-center text-white uppercase tracking-wider">
                            Email
                        </th>
                        <th class="w-40 px-4 py-3 text-xs font-medium text-center text-white uppercase tracking-wider">
                            Created At
                        </th>
                        <th class="w-40 px-4 py-3 text-xs font-medium text-center text-white uppercase tracking-wider">
                            Last Seen
                        </th>
                        <th class="w-24 px-4 py-3 text-xs font-medium text-center text-white uppercase tracking-wider">
                            Status
                        </th>
                        <th class="w-32 px-4 py-3 text-xs font-medium text-center text-white uppercase tracking-wider">
                            Actions
                        </th>
                    </tr>
                </thead>
                    ...
                    @forelse ($admins as $index => $admin)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-center">
                                {{-- @if ($admin->adminsProfile && $admin->adminsProfile->profile_image && $admin->adminsProfile->profile_image !== 'blank_profile.png')
                                    <img src="{{ asset('storage/' . $admin->adminsProfile->profile_image) }}"
                                ...
                                <img class="h-10 w-10 rounded-full object-cover border-2 border-indigo-100"
                                    src="{{ asset('/storage/' . $admin->adminsProfile->profile_image) }}"
                                    alt="Profile image">
        </div>
        </td>
        ...
        <td class="px-4 py-4 text-sm text-center text-green-600 font-semibold">
            Active
        </td>
        ...
        <td class="px-4 py-4 text-sm text-center">
            <div class="flex justify-center space-x-2">
                <a href="{{ route('admin.admins.show', $admin->id) }}"
                    class="p-2 text-amber-600 bg-amber-50 rounded-md hover:bg-amber-100" title="Lihat Detail">
                ...
                <a href="{{ route('admin.admins.edit', $admin->id) }}"
                    class="p-2 text-indigo-600 bg-indigo-50 rounded-md hover:bg-indigo-100" title="Edit">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path
        ...
        <div class="relative w-full max-w-md max-h-full p-4">
            <div class="relative bg-white rounded-lg shadow-lg">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-medium text-gray-800">Apakah anda yakin ingin delete all admin?</h3>
    ...
    <div data-popover id="popover-export" role="tooltip"
        ...
        <div class="px-3 py-2 bg-blue-50 border-b border-gray-200 rounded-t-lg">
            <h3 class="font-semibold text-blue-600">Export Admin</h3>
        </div>
        <div class="px-3 py-2">
            <p>Tindakan ini akan membuat file excel dari data admin.</p>
        </div>
        <div data-popper-arrow></div>
    </div>
