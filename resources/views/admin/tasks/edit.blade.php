...
<div class="p-4 bg-white rounded-lg shadow-md max-w-4xl mx-auto mt-6">
    <h2 class="text-xl font-bold mb-4 text-brandBlue">Edit Tugas / Soal</h2>
    ...
    <form action="{{ route('admin.tasks.update', $task->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="batch_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    ...
                    @foreach($batches as $batch)
                        <option value="{{ $batch->id }}" {{ old('batch_id', $task->batch_id) == $batch->id ? 'selected' : '' }}>{{ $batch->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="category" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    <option value="ALL" {{ old('category', $task->category) == 'ALL' ? 'selected' : '' }}>Semua / Access Level A</option>
                    <option value="HOTS" {{ old('category', $task->category) == 'HOTS' ? 'selected' : '' }}>HOTS</option>
                    <option value="PCK" {{ old('category', $task->category) == 'PCK' ? 'selected' : '' }}>PCK</option>
                    <option value="LITERASI_NUMERASI" {{ old('category', $task->category) == 'LITERASI_NUMERASI' ? 'selected' : '' }}>Literasi & Numerasi</option>
                </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required value="{{ old('title', $task->title) }}">
        ...
        <div class="mb-4">
            ...
            <textarea name="body" id="editor" rows="6" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('body', $task->body) }}</textarea>
            @error('body') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Gambar Soal</label>
            @if($task->image_path)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $task->image_path) }}" alt="Gambar Soal" class="h-32 object-contain bg-gray-50 border rounded p-1">
                </div>
            @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div>
                ...
                <input type="datetime-local" name="starts_at" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required value="{{ old('starts_at', $task->starts_at->format('Y-m-d\TH:i')) }}">
            ...
            <div>
                ...
                <input type="datetime-local" name="ends_at" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required value="{{ old('ends_at', $task->ends_at->format('Y-m-d\TH:i')) }}">
            ...
            <div>
                ...
                <input type="number" name="max_submissions" min="1" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required value="{{ old('max_submissions', $task->max_submissions) }}">
        ...
        <div class="flex justify-end gap-2 mt-6 pt-4 border-t border-gray-100">
            ...
            <button type="submit" class="px-4 py-2 bg-brandBlue text-white rounded hover:bg-brandBlue-dark">Update Tugas</button>
        </div>
    </form>
</div>
