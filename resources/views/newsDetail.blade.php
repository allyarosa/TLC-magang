            ...
            <div class="bg-white rounded-2xl shadow-md overflow-hidden">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    @if (isset($news->category))
                        <span
                            class="inline-block px-3 py-1 bg-indigo-100 text-indigo-800 text-xs font-medium rounded-full mb-4">
                    ...
                    <div class="flex items-center mb-8">
                        ...
                        @if (isset($news->author) && isset($news->author->avatar))
                            <img src="{{ asset('storage/' . $news->author->avatar) }}"
                                alt="{{ $news->author->name ?? 'Admin' }}" class="w-10 h-10 rounded-full mr-3">
                        @else
                            <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center mr-3">
                                <span
                                    class="text-indigo-600 font-bold">{{ substr($news->author->name ?? 'A', 0, 1) }}</span>
                            </div>
                        @endif
                    ...
                    @if (isset($news->tags) && count($news->tags) > 0)
                        <div class="mt-6 mb-8">
                            @foreach ($news->tags as $tag)
                                <span
                                    class="inline-block bg-gray-100 rounded-full px-3 py-1 text-sm font-medium text-gray-700 mr-2 mb-2">
                                    #{{ $tag->name }}
                                </span>
                            @endforeach
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ($relatedNews as $item)
                            <a href="{{ route('news.show', $item->slug) }}"
                                ...
                                @endif
                                <div class="p-4">
                                    <h3 class="text-lg font-semibold text-gray-800 mb-2 line-clamp-2">{{ $item->title }}
                                    </h3>
